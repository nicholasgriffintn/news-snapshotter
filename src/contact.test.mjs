...
test('source URLs are included in delivered messages', async (context) => {
	...
	await sendContactMessage(request(contactBody({ sourceUrl: 'https://publisher.example/story' })), env);
...
test('delivery failures return a service error', async (context) => {
	...
	const { env } = environment({ CONTACT_EMAIL: { send: async () => Promise.reject(new Error('down')) } });
